...
{
    // OpenGl default setting only render one face, only disable it during this funciton
    ...
    double step = 1.0; // each grid steps is 1
    for (double x = -2; x <= 2; x += step) {
        for (double y = -2; y <= 2; y += step) {

            double z1 = (-a * x - b * y - d) / c;
            double z2 = (-a * (x + step) - b * y - d) / c;
            double z3 = (-a * x - b * (y + step) - d) / c;
